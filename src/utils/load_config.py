...
from langchain.chat_models import ChatOpenAI  # OpenAI instead of AzureChatOpenAI
...
# commented this because i dont have azure AzureOpenAI api-key
    ...
    def load_llm_configs(self, app_config):
        self.model_name = os.getenv("GPT_MODEL_NAME", "gpt-4")  # Default to GPT-4
        self.agent_llm_system_role = app_config["llm_config"]["agent_llm_system_role"]
        self.rag_llm_system_role = app_config["llm_config"]["rag_llm_system_role"]
        self.temperature = app_config["llm_config"]["temperature"]
        self.embedding_model_name = os.getenv("OPENAI_EMBED_MODEL", "text-embedding-ada-002")  # Default embedding model

    def load_openai_models(self):
        openai_api_key = os.environ["OPENAI_API_KEY"]

        # This will be used for text generation
        ...
        # This will be used for embeddings
